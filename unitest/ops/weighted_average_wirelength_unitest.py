...
from dreamplacefpga.ops.weighted_average_wirelength import weighted_average_wirelength
sys.path.pop()
...
class WeightedAverageWirelengthOpTest(unittest.TestCase):
    def test_weighted_average_wirelength_random(self):
        ...
        #net_weights = np.array([1, 1], dtype=np.float32)
        ...
        pin_pos = np.array([[103.7478, 127.4158], [103.5688, 127.7118], [103.6268, 127.5448], [103.804, 127.548], [103.462, 127.399], [103.502, 127.211], [103.4118, 127.4038], [103.6178, 127.8198]], dtype=np.float32)

        net2pin_map = np.array([np.array([0, 1, 2, 3]), np.array([4, 5, 6, 7])])
        pin2net_map = np.zeros(len(pin_pos), dtype=np.int32)
        for net_id, pins in enumerate(net2pin_map):
            for pin in pins:
                pin2net_map[pin] = net_id
        ...
        #elfPlace Impl
        xy_max = [pin_pos[net2pin_map[i]].max(axis=0) for i in range(2)]
        xy_min = [pin_pos[net2pin_map[i]].min(axis=0) for i in range(2)]
        ...
        yexpPosY = []
        wl = []
        for i, pin in enumerate(net2pin_map):
            expNegX.append(np.e**(-(pin_pos[pin][:,0] - xy_min[i][0])/gamma))
            expPosX.append(np.e**((pin_pos[pin][:,0] - xy_max[i][0])/gamma))
            expNegY.append(np.e**(-(pin_pos[pin][:,1] - xy_min[i][1])/gamma))
            expPosY.append(np.e**((pin_pos[pin][:,1] - xy_max[i][1])/gamma))
            xexpNegX.append(pin_pos[pin][:,0] * expNegX[i])
            xexpPosX.append(pin_pos[pin][:,0] * expPosX[i])
            yexpNegY.append(pin_pos[pin][:,1] * expNegY[i])
            yexpPosY.append(pin_pos[pin][:,1] * expPosY[i])
    
        #pdb.set_trace()
        for i in range(2):
            wl.append(xexpPosX[i].sum()/expPosX[i].sum() - xexpNegX[i].sum()/expNegX[i].sum() + yexpPosY[i].sum()/expPosY[i].sum() - yexpNegY[i].sum()/expNegY[i].sum())

        totalWl = np.sum([np.sum(i) for i in wl])
        print("elfPlace WL = ", totalWl)
        ...
        net_bounding_box_min = np.ones_like(net_mask, dtype=np.float32)
        net_bounding_box_min *= pin_pos.max()
        net_bounding_box_max = np.zeros_like(net_mask, dtype=np.float32)
        ...
        custom = weighted_average_wirelength.WeightedAverageWirelength(
                ...
                net_bounding_box_min=torch.from_numpy(net_bounding_box_min),
                net_bounding_box_max=torch.from_numpy(net_bounding_box_max),
                num_threads=1,
        ...
        np.testing.assert_allclose(result.data.numpy(), golden_value, atol=1e-4)
        ...
        np.testing.assert_allclose(result.data.numpy(), golden_value, atol=1e-4)
        np.testing.assert_allclose(grad_atomic.data.numpy(), grad.data.numpy(), rtol=1e-4, atol=1e-4)
        ...
        np.testing.assert_allclose(result.data.numpy(), golden_value, atol=1e-4)
        np.testing.assert_allclose(grad_merged.data.numpy(), grad.data.numpy(), rtol=1e-4, atol=1e-4)
        ...
        if torch.cuda.device_count():
            ...
            np.testing.assert_allclose(result_cuda.data.cpu().numpy(), golden_value, atol=1e-4)
            np.testing.assert_allclose(grad_cuda.data.cpu().numpy(), grad.data.numpy(), rtol=1e-4, atol=1e-4)
