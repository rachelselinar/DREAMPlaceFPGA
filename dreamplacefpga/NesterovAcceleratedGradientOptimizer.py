...
class NesterovAcceleratedGradientOptimizer(Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for i, prm in enumerate(group['params']):
                if prm.grad is None:
                    continue
                if not group['u_k']:
                    group['u_k'].append(prm.data.clone())
                    # directly use prm as v_k to save memory
                    #group['v_k'].append(torch.autograd.Variable(prm.data, requires_grad=True))
                    group['v_k'].append(prm)
                ...
                # I do not know how to copy u_k back to prm when exit yet
                #prm.data.copy_(v_k.data)
