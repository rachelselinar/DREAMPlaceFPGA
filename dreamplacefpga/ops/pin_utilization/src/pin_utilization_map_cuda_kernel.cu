    ...
    if (i < num_nodes)
    {
        ...
        for (int x = bin_index_xl; x < bin_index_xh; ++x)
        {
            for (int y = bin_index_yl; y < bin_index_yh; ++y)
            {
                ...
                atomic_add_op(&pin_utilization_map[index], overlap * density);
            }
        }
    }
}
...
#define REGISTER_KERNEL_LAUNCHER(T)                                                         \
    template int pinDemandMapCudaLauncher<T>(                                               \
            const T *node_x, const T *node_y, const T *node_size_x, const T *node_size_y,   \
            const T *half_node_size_stretch_x, const T *half_node_size_stretch_y,           \
            const T *pin_weights, const T xl, const T yl, const T xh, const T yh,           \
            const T bin_size_x, const T bin_size_y, const int num_bins_x,                   \
            const int num_bins_y, const int num_nodes, bool deterministic_flag,             \
            T *pin_utilization_map                                                          \
            );
